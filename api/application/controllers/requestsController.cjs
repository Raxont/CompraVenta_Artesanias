...
class RequestController {
    ...
    async getRequestsByUserId(req, res) {
        try {
            const requests = await this.requestService.getRequestsByUserId(req.params.id);
            res.status(200).json(requests);
        } catch (error) {
            const errorObj = JSON.parse(error.message);
            res.status(errorObj.status).json({ message: errorObj.message });
    ...
    async createRequest(req, res) {
        try {
            ...
            const  {usuarioId, carrito, aPagar} = req.body;

           
            const request = await this.requestService.createRequest(usuarioId, carrito, aPagar);
    ...
    async updateRequest(req, res) {
        try {
            const errors = validationResult(req);
            if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
            const request = await this.requestService.updateRequest(req.params.id, req.body);
            res.status(200).json(request);
        } catch (error) {
            const errorObj = JSON.parse(error.message);
            res.status(errorObj.status).json({ message: errorObj.message });
    ...
    async searchRequests(req, res) {
        try {
            const requests = await this.requestService.searchRequestsByName(req.query.name);
            res.json(requests);
        } catch (error) {
