...
class RequestService {
    ...
    async getRequests() {
        ...
        if (!request) {
            throw new Error(JSON.stringify({status: 404, message: 'Request not found'}));
        }
        return request;
    }
    ...
    async createRequest(usuarioId, carrito, aPagar) {
        ...
        if (!acknowledged) return {error: true, message:'No se pudo crear el pedido...'}
        const paymentData = {
            usuarioId: new ObjectId(usuarioId),
    ...
    async updateRequest(id, data) {
        ...
        if (!updatedRequest) {
            throw new Error(JSON.stringify({status: 404, message: 'Request not found or could not be updated'}));
        }
    ...
    async deleteRequest(id) {
        ...
        if (!deletedRequest) {
            throw new Error(JSON.stringify({status: 404, message: 'Request not found or could not be deleted'}));
        }        
