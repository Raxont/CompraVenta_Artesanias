...
class CouponsService {
  ...
  async getCoupons() {
    ...
    if (!Coupons) {
      throw new Error(
        JSON.stringify({ status: 404, message: "Coupons not found" })
      );
    }
    return Coupons;
  }
  ...
  async updateCoupons(id, data) {
    const updatedCoupons = await this.couponsRepository.updateByCode(id, data);
    if (!updatedCoupons) {
      throw new Error(
        JSON.stringify({
          status: 404,
          message: "Coupons not found or could not be updated",
        })
      );
    }
  ...
  async deleteCoupons(id) {
    ...
    if (!deletedCoupons) {
      throw new Error(
        JSON.stringify({
          status: 404,
          message: "Coupons not found or could not be deleted",
        })
      );
    }
